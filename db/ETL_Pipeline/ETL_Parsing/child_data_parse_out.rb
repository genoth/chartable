...
def construct_child_table(original_csv_file, child_table_headers, parent_table_data)
  # p "method start"
  # i = 1
  ...
  CSV.foreach(original_csv_file, headers: true, header_converters: :symbol) do |original_row|
    ...
    # Now what we have to do is clone the data from the original csv file, except for the necessary foreign key changes.
      ...
      # Prep data for new child table
        # make headers array for new child table (subtract child_table_headers from original_csv_headers)
...
construct_child_table(original_csv_file, child_table_headers, parent_table_data)
