...
def construct_parent_table(original_csv_file, child_table_headers, child_table_data)
  # p "method start"
  # i = 1

  original_csv_headers = CSV.read(original_csv_file, headers: true, header_converters: :symbol).headers
  new_table_headers = (original_csv_headers - child_table_headers)
  ...
  CSV.foreach(original_csv_file, headers: true, header_converters: :symbol) do |original_row|
    ...
    child_table_headers.each do |original_csv_column|
      child_table_search_term.store(original_csv_column, original_row[original_csv_column])
    end
    foreign_key_value = child_table_data.index(child_table_search_term) + 1
    ...
    # Now what we have to do is clone the data from the original csv file, except for the necessary foreign key changes.
      # p original_csv_headers
      # p child_table_headers
      # p new_table_headers
      ##########################
      # Prep data for new parent table
        # make headers array for new parent table (subtract child_table_headers from original_csv_headers)
